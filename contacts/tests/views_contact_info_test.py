...
class SuccessUrlTests(TestCase):
    ...
    def test_views_with_success_url_is_login_redirect_url(self):
        views_with_home_success_url = [UpdatePersonalInfo]
        for view in views_with_home_success_url:
            self.assertEqual(view().get_success_url(),
                             reverse(settings.LOGIN_REDIRECT_URL),
                             "%s does not have LOGIN_REDIRECT_URL as success_url"
                             % view)
...
class AddContactTests(TestCase):
    ...
    def test_has_expected_permissions_properties(self):
        self.assertEqual(self.view.permission_required, 'registration.add_user')
...
class DeleteContactTests(TestCase):
    ...
    def test_has_expected_permissions_properties(self):
        view = DeleteContact()
        self.assertEqual(view.permission_required, 'registration.delete_user')
...
class UpdateContactTests(TestCase):
    def test_has_expected_permissions_properties(self):
        view = UpdateContact()
        self.assertEqual(view.permission_required, 'registration.add_user')
        self.assertTrue(view.raise_exception)
...
def test_update_contact_form_invalid_adds_a_message_to_messages(rf):
    ...
    view.request = r
    form = mock.MagicMock()
    with mock.patch('contacts.views.messages') as messages:
